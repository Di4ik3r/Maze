...
public class Unit
{
    ...
    public Unit(Vector2 position, int birthDay)
    {
        ...
        this.position = new Vector3(position.x,
                                    Unit.objectMap[(int)position.x, (int)position.y].transform.position.y + this.meshHeight,
                                    position.y);
    }
    ...
    private Vector3 MoveLogic()
    {
        ...
        bool available = Unit.digitalMap[(int)provedChoices[choice].x, (int)provedChoices[choice].y] == 0 ? true : false;
        if (!available)
        {
        ...
        if (!available)
        {
            ...
            provedChoices[choice] = stepHistory[stepHistory.Count - 1];
        ...
        y = Unit.objectMap[(int)x, (int)z].transform.position.y + this.meshHeight;

        Unit.digitalMap[(int)x, (int)z] = 9;
